...
from genai_api import call_general_ai, analyze_notes_api, chatbot_reply_api, generate_quiz_api, generate_learning_path_ai
...
def api_generate_quiz():
    ...
    response = generate_quiz_api(subject, class_level)
    if response:
...
def chatbot_send():
    ...
    reply   = chatbot_reply_api(msg, subject=subject, role='student') or 'Add Gemini API key'
...
def chatbot_send():
    ...
    msg     = data.get('message', '').strip()
    subject = data.get('subject', 'General')
    reply   = chatbot_reply_api(msg, subject=subject, role='teacher') or 'Add Gemini API key'
    db.session.add(ChatMessage(user_id=current_user.id, role='user', content=msg, subject=subject))
    db.session.add(ChatMessage(user_id=current_user.id, role='assistant', content=reply, subject=subject))
    db.session.commit()
    return jsonify({'reply': reply})
